...
if (window.top === window.self) {
    ...
    function startAnalysis(messageId, email) {
        chrome.storage.sync.get({ accountsAuthState: {} }, (items) => {
            ...
            import(analysisCoreUrl).then(({ initializeUI }) => {
                ...
                chrome.runtime.sendMessage({ type: 'GET_RAW_MESSAGE_OUTLOOK', messageId, email });
            });
        });
    }

    function findUserEmailFromDOM() {
        // Try to find user email from Outlook DOM
        // Outlook uses various selectors, we'll try common ones
        
        // Try profile button
        const meButton = document.querySelector('[data-testid="me-button"]');
        if (meButton) {
        ...
        // Try meta tags
        const metaUpn = document.querySelector('meta[name="userPrincipalName"]');
        if (metaUpn && metaUpn.content) return metaUpn.content.toLowerCase();

        return null;
    }

    function findMessageIdFromDOM() {
        // Outlook web uses different URL structure than Gmail
        // The message ID is typically in the URL hash or query parameters
        
        // Try URL hash format: #/mail/id/AAQkAD...
        const hash = window.location.hash;
        if (hash) {
            const match = hash.match(/\/mail\/id\/([^\/\?]+)/);
            if (match && match[1]) return match[1];
        }

        // Try data attributes on message elements
        const messagePane = document.querySelector('[role="region"][aria-label*="message"]');
        if (messagePane) {
            const itemId = messagePane.getAttribute('data-convid') || messagePane.getAttribute('data-itemid');
            if (itemId) return itemId;
        }

        // Try reading pane
        const readingPane = document.querySelector('[id^="ReadingPaneContainerId"]');
        if (readingPane) {
            const itemId = readingPane.getAttribute('data-itemid');
            if (itemId) return itemId;
        }
    ...
    new MutationObserver(() => {
        const messageId = findMessageIdFromDOM();
        ...
        if (messageId && email) {
            if (messageId !== currentMessageId || email !== currentUserEmail) {
                ...
                // Find a suitable element to attach the score UI
                // Outlook structure varies, try common locations
                let fromElement = document.querySelector('[role="heading"][aria-level="2"]');
                if (!fromElement) fromElement = document.querySelector('.ReadingPaneContainer');
                if (!fromElement) fromElement = document.querySelector('[data-testid="message-header"]');
    ...
    function injectAuthModal(email, state, msgId) {
        ...
        content.querySelector('#auth-modal-button').onclick = () => {
            ...
            const statusElement = document.getElementById("item-subject");
            if (statusElement) statusElement.innerText = "Waiting for Microsoft login...";

            chrome.runtime.sendMessage({
                type: 'TRIGGER_INTERACTIVE_AUTH_OUTLOOK',
                email: email,
